...
export function Profile(){
 ...
 async function save(event:React.FormEvent){event.preventDefault();setBusy(true);try{const response=await fetch('/api/profile',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({display_name:name,avatar})});const data=await response.json();if(!response.ok)throw Error(data.error);setName(data.profile.display_name);setAvatar(data.profile.avatar);window.dispatchEvent(new CustomEvent('pem:profile-updated',{detail:{avatar:data.profile.avatar}}));setStatus('Perfil salvo na sua conta.');}catch(error){setStatus(error instanceof Error?error.message:'Não foi possível salvar.');}finally{setBusy(false);}}
