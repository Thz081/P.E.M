...
- Produção atualizada no domínio `https://pem-monarcas.vercel.app` a partir da branch `codex/plataforma-estudos`. Commit funcional validado: `ea3dc96`. Rollback preservado: commit `5e1982b`, deployment `dpl_65QCMoeuWgmEfoKiu7XH16kpDPYe`.
...
- Evidência remota: CI `quality` verde; prévia `dpl_BPFKVtXCvem8kozneRkH1mv1kADJ` READY; desktop e mobile conferidos; roteiro com 9 etapas; contador 45/52 em 24/09; sem overflow, 5xx ou erros de runtime; `/api/admin` 403 e `/api/chat` 401 na demonstração.
