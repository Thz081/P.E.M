# Retomada exata — estado confirmado até 24/09/2026 às 09:51 (Brasília)
...
Guia final do professor atualizado com acesso `visitante` / `monarcas`, recursos atuais, limitações e roteiro de avaliação. Às 09:50, o PDF sem data na capa foi reenviado, conferido em 9 páginas e substituiu o arquivo antigo no mesmo ID do Drive. Próxima execução deve retomar P16/P17 sem reabrir a porta de publicação já concluída, salvo regressão comprovada.

## Plano e faltas — próxima semana
Kanban: 17/26 concluídas, nove abertas (P16–P24). Ordem: revisar vínculos/categorias de P16; processar a fila privada P17 em lotes, partindo de 311 páginas (286 sem tentativa, 24 aguardando visual e uma com fórmulas para corrigir); fazer curadoria e benchmark de 30 perguntas em P18; somente então validar tutor P19 e feedback P20 com injeção, recusa sem fonte, evidências e privacidade; P22 pode avançar em paralelo quando o caderno-fonte estiver disponível; P23/P24 depois de definir regras do simulado e acessibilidade; P21 apenas se existir fallback gratuito autorizado e com orçamento confirmado.

Pedido adicional para a próxima semana: reposicionar o site como plataforma de estudos para o ENEM, retirar do produto a narrativa/identidade da Ordem da Fênix e a Taça, remover a visita guiada específica de professor e manter a demonstração de visitante para estudantes. Auditar rotas, navegação, links, ativos, dados e guias antes de remover; preservar histórico interno e planejar backup/migração se houver dados associados. Expandir substancialmente conteúdo e resumos por matéria, remover/substituir a captura incompleta de WhatsApp hoje chamada “resumo gráfico” e priorizar material original/fontes autorizadas. Ferretto pode inspirar formato, sem cópia protegida sem licença.

Permanecem fora do aceite: teste real do tutor e feedback de redação (IA continua desligada), OCR/revisão das centenas de páginas, 40 questões adicionais, simulado e relatório. Os dois responsáveis ainda precisam ativar por conta própria os logins separados de aluno/admin. Supabase Free não oferece verificação de senhas vazadas; a aplicação exige senha forte.

Cota informada neste corte: 6% da janela de 5 horas e 7% semanal. Luna médio para lotes/documentação; Sol médio para ambiguidade/depuração; Astra só para risco crítico. Esta passagem não autoriza gasto em lote ou chamadas IA reais. As notas antigas abaixo ficam preservadas como histórico e não substituem este estado atual.
...
P17 agora tem OCR seletivo real em WSL (`scripts/apply_ocr_review.py`) e registro de conferência visual (`scripts/confirm_ocr_review.py`). 12 capas/divisórias conferidas; 25 páginas de Biologia receberam OCR, 24 aguardam visual e uma exige correção de fórmulas. Fila privada atual 311, das quais 286 ainda sem tentativa. Seis testes e reconstrução de fila passaram. P17 segue aberta; fonte não autorizada para IA. Ver `docs/TRIAGEM-P17.md`.

## Marco anterior — P07 concluída; P25/P26 como porta de publicação
